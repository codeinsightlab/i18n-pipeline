...
function testHtmlReportCommand() {
  ...
  assert.equal(typeof json.summary.hits_total_scan_occurrences, "number");
  assert.equal(typeof json.summary.extractable_hits_total, "number");
  assert.equal(typeof json.summary.extracted_key_unique_total, "number");
  assert.equal(typeof json.summary.apply_preview_replacement_occurrences, "number");
  assert.equal(typeof json.summary.apply_preview_key_unique_total, "number");
  assert.equal(typeof json.summary.apply_preview_auto_key_unique_count, "number");
  assert.equal(typeof json.summary.apply_preview_auto_replacement_occurrences, "number");
  assert.equal(typeof json.summary.auto_keys, "number");
  assert.equal(typeof json.summary.conflicts, "number");
  assert.equal(Array.isArray(json.groups), true);
  assert.equal(json.groups.some((item) => item.group === "query"), true);
  const groupedExtractHits = json.groups.reduce((sum, item) => sum + Number(item.extract_hit_occurrences ?? 0), 0);
  const groupedApplyReplacements = json.groups.reduce((sum, item) => sum + Number(item.apply_preview_replacement_occurrences ?? 0), 0);
  const groupedUniqueKeys = json.groups.reduce((sum, item) => sum + Number(item.key_unique_count ?? 0), 0);
  assert.equal(groupedExtractHits, json.summary.extractable_hits_total);
  assert.equal(groupedApplyReplacements, json.summary.apply_preview_replacement_occurrences);
  assert.equal(groupedUniqueKeys, json.summary.extracted_key_unique_total);
  assert.equal(Array.isArray(json.conflicts), true);
  assert.equal(Array.isArray(json.autos), true);
  assert.equal(new Set(json.autos.map((item) => item.auto_key)).size, json.autos.length);
  assert.equal(json.autos.every((item) => Array.isArray(item.apply_preview_samples)), true);
